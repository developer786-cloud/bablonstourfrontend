...
const TravelConsultationPopup = () => {
  ...
  return (
    <div className="fixed inset-0 z-[1200] flex items-center justify-center bg-dark-900/74 px-3 py-3 backdrop-blur-sm sm:px-6 sm:py-4" role="presentation">
      <div
        aria-labelledby="consultation-popup-title"
        aria-modal="true"
        className="relative grid max-h-[94vh] w-full max-w-5xl overflow-y-auto rounded-2xl bg-[#fffaf2] shadow-[0_32px_90px_rgba(0,0,0,0.34)] ring-1 ring-white/70 sm:rounded-[1.25rem] lg:grid-cols-[1.04fr_0.96fr] lg:overflow-hidden"
        role="dialog"
      >
        <button
          type="button"
          ...
          className="absolute right-3 top-3 z-20 flex h-10 w-10 items-center justify-center rounded-full bg-white text-dark-900 shadow-lg ring-1 ring-dark-900/10 transition hover:bg-sand-50 sm:right-4 sm:top-4 sm:h-11 sm:w-11"
        >
          <FaTimes className="h-4 w-4" />
        </button>

        <div className="relative flex flex-col px-4 py-5 sm:px-8 sm:py-6 lg:px-10 lg:py-8">
          <div className="pointer-events-none absolute left-0 top-0 h-44 w-44 rounded-br-full bg-white/70" />
          <div className="relative flex flex-wrap items-start justify-between gap-3 pr-10 sm:gap-4 sm:pr-0">
            <img src={logo} alt="Bablons Travel & Entertainment" className="h-14 w-auto object-contain sm:h-20" />
            <span className="inline-flex items-center gap-2 rounded-full bg-secondary-500 px-3 py-2 text-[0.66rem] font-black uppercase tracking-[0.04em] text-white shadow-[0_12px_24px_rgba(217,111,58,0.25)] sm:px-4 sm:text-xs">
          ...
          <div className="relative mt-5">
            <p className="inline-flex items-center gap-2 rounded-full border border-accent-200 bg-white/80 px-3 py-2 text-[0.62rem] font-black uppercase tracking-[0.04em] text-accent-700 shadow-sm sm:px-4 sm:text-[0.7rem]">
              <FaCrown className="h-3.5 w-3.5" />
              Trusted by 500+ happy travelers
            </p>
            <h2 id="consultation-popup-title" className="mt-4 font-display text-[2.35rem] font-bold leading-[1] text-dark-900 sm:text-[4.25rem]">
              Your Dream Holiday
              <span className="block text-accent-500">Starts Here!</span>
            </h2>
            <p className="mt-3 max-w-xl text-sm font-semibold leading-6 text-sand-700 sm:mt-4 sm:text-lg sm:leading-7">
          ...
          <div className="mt-6 grid grid-cols-2 gap-3 sm:mt-7 sm:grid-cols-3 sm:gap-4">
            {services.map((service) => {
              ...
              return (
                <div key={service.title} className="flex min-w-0 items-center gap-2.5 sm:gap-3">
                  <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full border border-accent-200 bg-white text-accent-600 shadow-sm sm:h-12 sm:w-12">
                    <Icon className="h-4 w-4 sm:h-5 sm:w-5" />
                  </span>
                  <strong className="min-w-0 text-xs font-black leading-tight text-dark-900 sm:text-sm">{service.title}</strong>
                </div>
              )
            })}
          </div>
          ...
          <div className="mt-5 grid gap-2 rounded-xl border border-accent-200 bg-white/85 p-2.5 shadow-[0_12px_30px_rgba(16,39,36,0.08)] sm:p-3">
            <a href={COMPANY_CONTACT.phoneHref} className="flex min-h-14 items-center gap-3 rounded-lg bg-white px-3 py-2 shadow-sm ring-1 ring-accent-100 transition hover:bg-accent-50">
              <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-accent-50 text-accent-600 ring-1 ring-accent-100">
                <FaPhoneAlt className="h-4 w-4" />
              </span>
              <span className="min-w-0 text-sm leading-tight text-dark-900">
                <span className="font-bold text-sand-600">Call Us: </span>
                <strong className="whitespace-nowrap font-black">{COMPANY_CONTACT.phoneDisplay}</strong>
              </span>
            </a>
            <a href={COMPANY_CONTACT.emailHref} className="flex min-h-14 items-center gap-3 rounded-lg bg-white px-3 py-2 shadow-sm ring-1 ring-accent-100 transition hover:bg-accent-50">
              <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-accent-50 text-accent-600 ring-1 ring-accent-100">
                <FaEnvelope className="h-4 w-4" />
              </span>
              <span className="min-w-0 text-sm leading-tight text-dark-900">
                <span className="font-bold text-sand-600">Email Us: </span>
